    ...
    async function fetchData() {
      try {
        const res = await axios.get(
          `http://localhost:8080/api/user/select-vendor?eventId=${eventId}`,
          {
            headers: {
            ...
            <ul className="space-y-6">
              {queries.map((query) => (
                ...
                >
                  ...
                  <p className="text-gray-400">
                    Event: {query.eventId?.title || "Untitled"} | Budget: ₹
                    {query.budget} | Date:{" "}
                  ...
                  {query.status === "Accepted" && (
                    <button
                      onClick={() =>
                        navigate(
                          `/rate/${query.vendorId._id}/${query.eventId._id}`
                        )
                      }
                      className="mt-3 px-4 py-2 bg-blue-200 hover:bg-blue-300 rounded text-black font-semibold"
                    >
                      ⭐ Rate Vendor
                    </button>
                  )}
              ...
              {vendors.map((vendor) => (
                ...
                >
                  ...
                  <button
                    ...
                    onClick={() =>
                      navigate(
                        `/event/${eventId}/vendor/${vendor._id}/query`
                      )
                    }
                  >
                    Query Vendor
