...
function SmartChecklist() {
  ...
  const addTask = async () => {
    try {
      ...
      setTasks([...tasks, res.data.checklistItem]);
      setNewTask('');
    } catch (err) {
  ...
  const updateTask = async (id, label) => {
    try {
      ...
      setCurrentlyEditingId(null); // exit edit mode
    } catch (err) {
          ...
          >
            <div className="flex items-center gap-3 w-full">
              ...
              <input
                type="text"
                value={task.label}
                disabled={currentlyEditingId !== task._id}
                onChange={(e) => {
                  const updatedLabel = e.target.value;
                  setTasks((prevTasks) =>
                    prevTasks.map((t) =>
                      t._id === task._id ? { ...t, label: updatedLabel } : t
                    )
                  );
                }}
                className={`bg-transparent border-none outline-none text-white text-sm font-medium tracking-wide w-full placeholder:text-gray-500 ${
                  currentlyEditingId !== task._id ? 'cursor-not-allowed' : ''
                }`}
              />
            </div>
            <div className="flex items-center gap-3 text-gray-400 text-xs font-medium ml-4">
              <div className="flex items-center gap-2 text-xs font-semibold ml-4">
                {currentlyEditingId === task._id ? (
                  ...
                  <button
                    className="px-3 py-1 rounded-full border border-yellow-400/30 text-yellow-300 bg-yellow-400/10 hover:bg-yellow-400/20"
                    onClick={() => setCurrentlyEditingId(task._id)}
                  >
                    Update
                  </button>
                )}
